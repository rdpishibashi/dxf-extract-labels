...
| `unclassified_editor_<fname>_<0..N>` | DataFrame | 「未確定ラベル」`st.data_editor` の編集状態（採用チェック列を含む、v1.6.0。ファイルごとに `UNCLASSIFIED_ROWS_PER_TABLE`（既定10行）ごとに分割、テーブル数はラベル数に応じて可変、キー末尾はテーブル番号、v1.6.5・v1.6.6）|
