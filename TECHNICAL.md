...
7. **「未確定ラベル」UI でのレビュー**: 6 で確定しなかった残りを「未確定ラベル」と
   ...
   機器符号として出力される。表示レイアウトは v1.6.5 参照。
...
| `unclassified_editor_<fname>_<0..2>` | DataFrame | 「未確定ラベル」`st.data_editor` の編集状態（採用チェック列を含む、v1.6.0。ファイルごとに最大3個のテーブルへ分割、キー末尾はテーブル番号、v1.6.5）|
