...
| `ref_designator.py` | 機器符号（候補）パターン・除外リスト・確定リスト・図面枠/フォーマットブロック検出・候補/確定/未確定ラベルの分類と集計（v1.6.0、確定リストは v1.6.3）|
...
| 相線・電源レール | 正規表現 | `^[LNP][0-9]+[A-Z]?$` | `L1`、`N24`、`P24` |
| PLC/内部信号名 | 正規表現 | `^X[A-Z]+$` | `XRST`、`XMCON`（`X1`等 `X+数字` は除外対象外） |
| 回路の説明 | 正規表現（v1.6.3） | キーワード+数字1桁まで許容（例 `^(?:GND\|POWER\|...)[0-9]?$`） | `GND`、`POWER`、`COM`、`OUT2`、`IN1`（`OUT12`等2桁以上は対象外） |
...
### 確定パターン（`CONFIRMED_PATTERN_CATEGORIES`、v1.6.3）

機器符号（候補）のうち、確実に Reference Designator と判定してよい形。一致した
ラベルは「未確定ラベル」UI でのレビューを経ずに自動採用される（`matched_confirmed_category()`
/ `is_confirmed_designator()` / `split_confirmed()`）。

| カテゴリ | 正規表現 | 例 |
|---------|--------|-----|
| letters_digits_2or3 | `^[A-Z]+[0-9]{2,3}$` | `R10`、`D100` |
| letters_digits_2or3_letter | `^[A-Z]+[0-9]{2,3}[A-Z]$` | `R10A`、`A007A` |
| hyphen_letters_digits_notail | `^[A-Z]+-[A-Z]+[0-9]+$` | `CN-IF21` |
| single_letter_digits_except_ab | `^[C-Z][0-9]+$` | `D1`、`D1000`（桁数不問） |
...
| `unclassified_editor_<fname>` | DataFrame | 「未確定ラベル」`st.data_editor` の編集状態（採用チェック列を含む、v1.6.0）|
