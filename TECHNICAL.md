...
| `ref_designator.py` | 機器符号（候補）パターン・除外リスト・図面枠/フォーマットブロック検出・候補/未確定ラベルの分類と集計（v1.6.0）|
...
5. **除外パターン適用**: 4 で一致したものから、除外パターン（後述）に該当するものを
   さらに除く。残ったものが「機器符号（候補）」、除外パターンに該当せずかつ
   パターン自体に一致しなかったものが「未確定ラベル」。
6. **図面枠が検出できない場合**: 警告を出し、図面枠フィルタなしでファイル全体
...
| シート | 内容 |
|--------|------|
| `Summary` | 全ファイルの集計（図面枠内ラベル数・機器符号（候補）数・未確定ラベル数〔未採用〕・図番等）。ファイル名セルは各ファイルシートへの内部ハイパーリンク |
| `Total` | 全ファイルの機器符号（候補、未確定ラベルからの採用分を含む）をユニーク集計した合計一覧（ラベル・個数）|
| `<ファイル名>` | ファイルごとの機器符号（候補）一覧（ラベル・個数）|
...
| オプション | 説明 |
|-----------|------|
| 機器符号（候補）以外も抽出 | OFF（既定）: 図面枠内・図面情報欄外のラベルから機器符号（候補）パターンに一致し除外パターンに非該当のものを抽出し、未一致は「未確定ラベル」として選択UIに回す。ON: 図面枠制限・フィルタなしで全ラベルを抽出（従来動作） |
...
### 除外パターン（`EXCLUSION_REGEXES` / `EXCLUSION_EXACT`）
...
| 普通名詞 | 完全一致（約100語） | 端子/スイッチの機能説明語 | `ALARM`、`MOTOR`、`GND`系は別カテゴリ |
| 回路の説明 | 完全一致 | 電源・接地・信号系統名 | `GND`、`POWER`、`COM` |
