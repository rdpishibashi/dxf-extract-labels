...
1. **図面枠検出**: `lineweight=100` の線分で囲まれた枠を検出（`detect_drawing_frames`）。
   ...
   **図面枠が見つからない場合も `analyze_dxf_regions()` の `labels` は
   埋める（v1.9.10）**: 従来、図面枠が見つからずエラーになる図面
   （`EE6892-455B.dxf` 等、上記の理由で枠が見つからないケースを含む）では
   `result['labels']` を空のまま返していた（「領域探索（面積比等の計算）は
   図面枠が無いと続行できない」という理由で、ラベル収集自体もまとめて
   諦めていた）。しかし `region_detector.build_region_results()`
   （app.py の「機器符号（候補）以外も抽出」ON時に使用）はこの `labels` の
   みを頼りにラベルを集計するため、複数図面を一括入力して「領域を検出」を
   使うと、図面枠が見つからない図面だけラベルが0件になり出力から消える
   不具合が発生した（2026-07-23 ユーザー報告）。`ref_designator.py` の
   `_collect_all_labels_fallback()` と同じ「枠制約なし・重複除去のみ」の
   方針で `labels` を埋めるよう修正（`frames`/`regions`/`error` の意味・
   挙動は変えていない＝領域探索自体は引き続き実施されない）。DXF-viewer
   の `core/region_detector.py` にも同じ変更を移植済み（ただしDXF-viewer
   側は元々 Model/Paper Space 自動選択〈上記〉が未移植のため、この移植は
   Model Space のみを対象にした従来の挙動の範囲内）。
