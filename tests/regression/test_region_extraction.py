...
PAPERSPACE_FRAME = _find_sample('EE5322-455-01B.dxf')  # 図面枠がPaper Spaceレイアウトに配置
...
@pytest.mark.skipif(not os.path.exists(PAPERSPACE_FRAME), reason='サンプル DXF が無い')
def test_frame_in_different_layout_than_content_is_not_cross_applied():
    """`EE5322-455-01B.dxf` は Model Space に実図面のジオメトリを持つが、
    図面枠線(lineweight=100/color=7)を直接の子に持つフォーマットブロック
    (JZB_0001)は Paper Space（"ICADSX Layout"）にのみ INSERT されている
    （frameとcontentがレイアウト的に分離した構造）。

    一時的な修正でこの Paper Space 側の図面枠を検出して使うようにしたところ、
    Model Space と Paper Space は独立した座標系であるため、Paper Space の
    図面枠bboxをModel Spaceのラベルに適用すると座標が対応せず、正しいラベルの
    大半が「枠外」と誤判定される不具合が発生した（`EE6892-455B.dxf`の`CB001`
    等。2026-07-14ユーザー報告）。そのため、Model Space に何らかの内容が
    あれば常にModel Spaceのみを対象にする方針に修正し、他レイアウトの図面枠を
    Model Spaceの内容に適用することはない。結果として、このファイルは
    （元の挙動どおり）図面枠が見つからないエラーになる——これは意図した
    仕様であり、退行ではない。"""
...
def test_connection_point_region_excluded():
    ...
    _, _, _, _, cps = _collect_region_geometry(doc, DEFAULT_REGION_CONFIG)
